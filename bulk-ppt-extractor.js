...
const zip = (filename) => {
    execSync('cd ' + toProcessDir + ' && cp ' + filename + '.pptx ../.' + outputDir + filename + '/' + filename + '.zip');
};
...
const extractImages = (filename) => {
    execSync('mv ' + outputDir + filename + '/ppt/media/* ' + outputDir + filename);
...
const movePPTAfterProcess = (filename) => {
    execSync('mv ' + toProcessDir + filename + '.pptx ' + processedDir + filename + '.pptx ');
};
